...
export class DatepickerPeriod {
  static monthNames = $localize`January|February|March|April|May|June|July|August|September|October|November|December`.split('|');
  static monthShortNames = $localize`Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec`.split('|');
  static dowNames = $localize`Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday`.split('|');
  static dowShortNames = $localize`Mo|Tu|Wd|Th|Fr|Sa|Su`.split('|');
  static shiftNames = $localize`Til Today|Til Yesterday|Til Weekend`.split('|');
  emptyReason: string;
  firstDayOfWeek = 7;
  end: Date;
  entryKey: string;
  ...
  get baseDate(): Date {
    switch (this.shiftDate) {
      ...
      default:
        return Utils.now;
      case 1:
        return Utils.addDateDays(Utils.now, -1);
      case 2:
        let diff = -(Utils.now.getDay() - this.firstDayOfWeek);
        while (diff > 0) {
          diff -= 7;
        }
        return Utils.addDateDays(Utils.now, diff);
    }
  }
  ...
  get isEmpty(): boolean {
    ...
    this.emptyReason = $localize`The period contains no evaluable days`;
